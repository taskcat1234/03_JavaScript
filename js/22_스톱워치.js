...
// START  버튼 클릭 시 수행되는 setInterval();
let currentInterval;
...
/* START 버튼이 클릭된 경우
    - START <-> PAUSE 버튼으로 변경
    - START가 클릭된 경우 10ms 마다 count 1씩 증가
    - count 증가 후 화면에 시간을 계산해서 출력(변도 함수)
*/
startBtn.addEventListener("click", () => {
  // START <-> PAUSE 버튼으로 변경
  if(startBtn.innerText === "PAUSE"){
    startBtn.innerText = "START"; // 스타트로 버튼 변경
    recordBtn.disabled = true // 버튼 비활성화

    // setInterval()을 꺠끗이 치워버림
    // -> clearInterval() 이용
    clearInterval(currentInterval); //멈춤
    return;
  }

  // startBtn.innerTest === "START" 인 경우
  startBtn.innerText = "PAUSE"; // 일시 정지로 버튼 변경
  recordBtn.disabled = false // 비활성화 취소
  // START가 클릭된 경우 10ms 마다 count 1씩 증가
...
/* reset 버튼이 클릭된 경우 
  ...
  - count = 0
  - clearInterval() 수행
  - 화면 숫자 00:00:00 ( 변도 함수 )
*/

resetBtn.addEventListener("click", () => {
  startBtn.innerText = "START"; // 스타트 버튼 형태로 변경
...
function displayOutput(){
  // 화면에 출력될 시간 분, 초, 1/100초 계산후 변수에 저장
