 ...
 * @param num : 입력 받은 수의 개수
 * @returns arr : 입력 받은 수가 저장된 배열
*/
...
btn6.addEventListener("click", function(){
  // createCounter() 호출시 반환 값
  // function(){
  //  count++;
  //  return count;
  // }

  // 반환된 함수를 ciunter에 저장(함수명 == counter)
...
btn7.addEventListener("click", function(){
  const resultFn = testFn7( function(a,b){return a+b;}); // 밑에 같은 이름의 함수 호출 testFn7
  console.log("resultFn(3) : ", resultFn(3)); //여기에 리턴 받은 90을 출력한다
});

function testFn7(otherFn){ //function(a,b){return a+b;}
  return function(num){ //return otherFn(10,20) -> 위에 값을 대입 a+b = 10+20 = 30
    return otherFn(10,20) *num; // 30의 값을 * num -> resultFn(3) 대입 90이 된다 이걸 위로 리턴해서
  }
};
...
arrows[3].addEventListener("click", () =>{
  ...
  const fn2 = () => {return{name:"홍길동",age:20}}; // const fn2 = () => {name:"홍길동",age:20}; <- 에러
  console.log(fn2())
});
...
document.querySelector("#btn8").addEventListener("click", function(){
  console.log(this); // === 이벤트가 발생한 요소 값이 불러와짐
});
