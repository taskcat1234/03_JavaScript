...
function letCheck(){
  ...
  /* 변수명 중복 X */
  //let name = "이순신"; /* 오류 발생!! */
  // 블럭 범위 변수 'name'을(를) 다시 선언할 수 없습니다.
...
{ // 함수 정의 시작
  ...
  // num3 변수에 저장된 값이 300이 같으면 {}블럭 수행
  if(num3 === 300){

    let num4 = 400;
    // -> let 는 {}블럭 레벨에서 선언된거라 내부에서만 사용가능
    // -> {}블럭을 벗어나면 사용불가능
  ...
  if(num3 === 300){
    ...
    let num4 = 4000;
    // 다른 블럭에서 같은 이름의 변수를 사용가능? YES
    console.log("두번째 if : ", num4);
  }
  /* 
  console.log("세번째 if : ", num4);
...
{ // 함수 정의 시작
  ...
  console.log("if 끝난 이후 fn2 값 : ", fn2); //함수 지역 변수 or 변경된 fn2 중 변경된 fn2가 출력된다.
...
function typeCheck(){
  ...
  console.log("sighgt : ", age,typeof sighgt);
  ...
  // 1) 배열(Array) : 변수의 묶음,
  //    묶인 변수에 0 부터 1씩 증가하는 수자를 부여(index)
  ...
  console.log( sumFn(3, 7) );
  console.log( sumFn(999, 9999) );
